...
| FD-1 occupied slot: a node that is absent at `rootPath` while its reserved slot already holds bytes is refused instead of claimed, and a concurrent raw write to that slot aborts the initializing transaction | Owned by `fdb-swift-bindings`: `DirectoryNodeWitnessTests.creationRefusesAnOccupiedSlot`, `concurrentSlotWriteAbortsCreation`; the resulting `invalidMetadata` reaches this adapter as `incompatibleStorageLayout` through the conversion in `FDBDirectoryAccess.swift` |
