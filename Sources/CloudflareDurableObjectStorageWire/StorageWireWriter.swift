...
public struct StorageWireWriter: Sendable {
    ...
    public static func encode<Failure: Error>(
        _ encode: (inout StorageWireWriter) throws(Failure) -> Void
    ) throws(Failure) -> ByteString {
        var measuringWriter = StorageWireWriter(measuring: ())
        try encode(&measuringWriter)
        let byteCount = measuringWriter.writtenByteCount
        var writer = StorageWireWriter(capacity: byteCount)
        try encode(&writer)
        precondition(
