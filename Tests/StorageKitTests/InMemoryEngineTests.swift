...
struct InMemoryEngineTests {
    ...
    @Test func clearRange() async throws {
        ...
        // [0x02, 0x04) を削除 → 0x02 と 0x03 が消える
        try await engine.withTransaction { tx in
    ...
    @Test func getRangeForward() async throws {
        ...
        try await engine.withTransaction { tx in
            ...
            #expect(collected.count == 3) // 0x02, 0x03, 0x04
    ...
    @Test func getRangeReverse() async throws {
        ...
        try await engine.withTransaction { tx in
            ...
            #expect(collected[0].key == [0x04]) // 逆順
    ...
    @Test func lexicographicOrdering() async throws {
        let engine = InMemoryEngine()

        // キーを逆順で挿入しても、range scan は辞書順で返る
        try await engine.withTransaction { tx in
