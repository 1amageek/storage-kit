...
/// Integration tests that verify database-framework's transaction contract.
///
...
@Suite("Database Framework Transaction Contract", .serialized, .enabled(if: PostgreSQLTestEnvironment.isConfigured))
struct DatabaseFrameworkTransactionContractTests {
    ...
    @Test func itemStorage_writeReadPattern() async throws {
        ...
        try await ActiveTransactionScope.$current.withValue(writeTx) {
            for user in users {
                ...
                try writeTx.setValue(data, for: key)
            }
    ...
    @Test func itemStorage_overwritePattern() async throws {
        ...
        // Initial write
        let tx1 = try engine.createTransaction()
        try tx1.setValue(serializeItem(name: "Original", email: "old@test.com"), for: key)
        try await tx1.commit()

        // Overwrite (update) - same key, new value
        let tx2 = try engine.createTransaction()
        try tx2.setValue(serializeItem(name: "Updated", email: "new@test.com"), for: key)
        try await tx2.commit()
    ...
    @Test func indexMaintenance_diffBasedUpdate() async throws {
        ...
        try await ActiveTransactionScope.$current.withValue(tx1) {
            ...
            try tx1.setValue([], for: idxKey) // Empty value = index marker
            try await tx1.commit()
        }
        ...
        try await ActiveTransactionScope.$current.withValue(tx2) {
            ...
            try tx2.setValue(serializeItem(name: "Alice", email: "alice2@test.com"), for: itemKeyBytes)
            ...
            try tx2.clear(key: oldIdxKey) // Remove old
            try tx2.setValue([], for: newIdxKey) // Add new
    ...
    @Test func collectRange_indexScanPattern() async throws {
        ...
        try await ActiveTransactionScope.$current.withValue(tx1) {
            for i in 0..<10 {
                ...
                try tx1.setValue(serializeItem(name: "User\(i)", email: "\(id)@test.com"), for: key)

                // Age index: ordered by age value
                let idxKey = indexKey(name: "user_age", value: age, id: id)
                try tx1.setValue([], for: idxKey)
            }
            try await tx1.commit()
        }
    ...
    @Test func bulkDeletion_clearAllTypeData() async throws {
        ...
        try await ActiveTransactionScope.$current.withValue(tx2) {
            ...
            try tx2.clearRange(beginKey: idxBegin, endKey: idxEnd)
    ...
    @Test func readYourWrites_overwriteWithinTransaction() async throws {
        ...
        try setup.setValue(serializeItem(name: "Original", email: "orig@test.com"), for: key)
        ...
        try await ActiveTransactionScope.$current.withValue(tx) {
            ...
            // Overwrite
            try tx.setValue(serializeItem(name: "Modified", email: "mod@test.com"), for: key)
    ...
    @Test func fullCRUDLifecycle() async throws {
        ...
        try await ActiveTransactionScope.$current.withValue(insertTx) {
            ...
            // Write index (name)
            try insertTx.setValue([], for: indexKey(name: "user_name", value: "Alice", id: userId))
            // Write index (email)
            try insertTx.setValue([], for: indexKey(name: "user_email", value: "alice@test.com", id: userId))
        ...
        try await ActiveTransactionScope.$current.withValue(updateTx) {
            ...
            try updateTx.clear(key: indexKey(name: "user_name", value: "Alice", id: userId))
            try updateTx.setValue([], for: indexKey(name: "user_name", value: "Alice Smith", id: userId))
        ...
        try await ActiveTransactionScope.$current.withValue(deleteTx) {
            ...
            try deleteTx.clear(key: indexKey(name: "user_name", value: "Alice Smith", id: userId))
            try deleteTx.clear(key: indexKey(name: "user_email", value: "alice@test.com", id: userId))
