...
struct EmbeddedBytesOwnershipTests {
    @Test func decodedFieldRetainsItsSourceFrame() throws {
        ...
        let releaseRecorder = EmbeddedByteReleaseRecorder()
    ...
    @Test func decoderAcceptsANonzeroIndexFrameView() throws {
        ...
        let response = try CloudflareDurableObjectStorageWireCodec
            .decodeResponse(view)
    ...
    private func decode(
        ...
        return try CloudflareDurableObjectStorageWireCodec
            .decodeResponse(frame)
    }

    private func makeOwnedBytes(
        _ bytes: [UInt8],
        releaseRecorder: EmbeddedByteReleaseRecorder
    ) -> ByteString {
        ByteString(
            retaining: EmbeddedTestByteAllocation(
                bytes: bytes,
                releaseRecorder: releaseRecorder
            )
        )
...
private final class EmbeddedTestByteAllocation: ByteStringOwner {
    let count: Int

    private let address: UInt
    private let releaseRecorder: EmbeddedByteReleaseRecorder
...
private final class EmbeddedByteReleaseRecorder: Sendable {
