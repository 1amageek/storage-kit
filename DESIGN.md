...
| Children | [StorageKit module](Sources/StorageKit/DESIGN.md); adapter modules `FDBStorage`, `SQLiteStorage`, `PostgreSQLStorage`, `CloudflareDurableObjectStorage` (module `DESIGN.md` files are added by the adapter conformance sprints K-30 through K-60 recorded in the workspace `PROGRESS.md`) |
...
| [SPEC.md](../SPEC.md) | product architecture authority | §7 capability list, §8 values and operations, §9 leases, §10.3 bootstrap, §24 verification | Defines what this package must guarantee. | Any deviation (for example the FDB nested-partition realization) is recorded in the owning module design, not silently absorbed. |
...
| `fdb-swift-bindings` | depends on (FDBStorage only) | `Database`, `Transaction`, `DirectoryLayer`, `DirectoryType` | Safe FoundationDB C client ownership. | Native partitions cannot nest; StorageKit Partitions on FDB are custom-typed directories (owned by the FDBStorage module design). |
